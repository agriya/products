...
class Attribute {
	...
	public function validateAttributeDetails($input_arr)
	{
		...
		$message = array('attribute_label.required' => 'Attribute Label required',
						'attribute_question_type.required' => 'Attribute type required'
						);
		...
		if($validator->passes())
		{
			...
			else
			{
				if(count($this->fields_arr) > 0)
				{
					...
					{
						return json_encode(array('status' => 'error', 'error_messages' => 'Attribute not found!'));
					}
					else
					{
		...
		if(isset($data_arr['attribute_question_type']))
		{
			if($attributes['attribute_question_type'] != $data_arr['attribute_question_type'])
			{
					...
					(in_array($attributes['attribute_question_type'], array_keys(\Config::get('products::ui_no_options'))) && in_array($data_arr['attribute_question_type'], array_keys(\Config::get('products::ui_options'))))))
				{
					...
					return json_encode(array('status' => 'error', 'error_messages' => 'The attribute options are in use, so attribute type cannot be changed.'));
	...
	public function deleteAttribute($attribute_id)
	{
		if($this->isOptionsAlreadyUsed($attribute_id))
		{
			return json_encode(array('status' => 'error', 'error_msg' => 'Attribute options are in use, so this attribute cannot be deleted.'));
		}
